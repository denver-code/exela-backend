...
database = database_client["exela-backend"]

users_db = database["users"]
messages_db = database["messages"]
...
async def find(_db, query, lenght=100000000000):
    cursor =  globals()[_db].find(query)
    return await cursor.to_list(length=lenght)
